...
for ip in remote_ips:
    ...
    for _ in range(2):
        # 2、同步代码
        git_cmd = f'{psexec_path} \\\\{ip} -u {username} -p {password} cmd /c "cd {repo_path} && git pull https://github.com/JunXu-Official/Polixir_Fapiao.git"'  # 你的git仓库的URL地址
        try:
    ...
    # 启动 Python 服务
    subprocess.Popen(f'cmd /c start cmd /k "conda activate {env_name} && python {service_script}"', cwd=work_dir, shell=True)
